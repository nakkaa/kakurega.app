...
<div v-else-if="!hardMuted" :class="$style.muted" @click="hideNote = false">
...
const appearNote = computed<Misskey.entities.Note | null>(() => isRenote ? note.value?.renote : note.value);
...
const isSensitive = defaultStore.state.hideNsfwNote && appearNote.value?.files?.some(x => x.isSensitive);
