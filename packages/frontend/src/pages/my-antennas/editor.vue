...
async function saveAntenna() {
	const includeKeywords = keywords.value.trim().split('\n').map(x => x.trim().split(' ').filter(k => k)).filter(x => x.length);

	if (!includeKeywords.length && src.value === 'all') {
		os.alert({
			type: 'error',
	...
	const includeUsers = users.value.trim().split('\n').map(x => x.trim()).filter(x => x);

	if (!includeUsers.length && src.value === 'users') {
		os.alert({
			type: 'error',
